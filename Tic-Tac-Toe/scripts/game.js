...
function startNewGame(){
    ...
    if(players[0].name ==='' || players[1].name === ''){
        alert('Please Enter the name of players first !')
        return ;
    }
...
function selectGameField(event){
    ...
    const selctedColumn = selectedField.dataset.col-1;
    const selctedRow = selectedField.dataset.row-1;
    if(gameData[selctedRow][selctedColumn]>0){
        alert('Please Select an Empty button !');
        return;
    }

    gameData[selctedRow][selctedColumn] = activePlayer +1;
